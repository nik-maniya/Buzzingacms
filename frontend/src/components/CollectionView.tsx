...
export function CollectionView({ collection, onBack, onEditItem, initialTab }: CollectionViewProps) {
      ...
      try {
        ...
        if (res.data.success && res.data.data && res.data.data.length > 0) {
          ...
          try {
            ...
            if (saved) {
              const parsed = parseTemplateContent(saved);
              setTemplateHtml(parsed.html);
              setTemplateCss(parsed.css);
              setTemplateJs(parsed.js);
            }
  ...
  const handleSaveTemplate = async () => {
    try {
      ...
      // Also save to localStorage as backup (combined for backward compatibility)
      try {
        ...
        localStorage.setItem(`collection-template-${collection.id}`, combinedHtml);
      } catch {}
    } catch (e: any) {
